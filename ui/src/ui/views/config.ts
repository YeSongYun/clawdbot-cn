...
function getSections(): Array<{ key: string; label: string }> {
  return [
    { key: "env", label: t("config.section.env", "Environment") },
    { key: "update", label: t("config.section.update", "Updates") },
    { key: "agents", label: t("config.section.agents", "Agents") },
    { key: "auth", label: t("config.section.auth", "Authentication") },
    { key: "channels", label: t("config.section.channels", "Channels") },
    { key: "messages", label: t("config.section.messages", "Messages") },
    { key: "commands", label: t("config.section.commands", "Commands") },
    { key: "hooks", label: t("config.section.hooks", "Hooks") },
    { key: "skills", label: t("config.section.skills", "Skills") },
    { key: "tools", label: t("config.section.tools", "Tools") },
    { key: "gateway", label: t("config.section.gateway", "Gateway") },
    { key: "wizard", label: t("config.section.wizard", "Setup Wizard") },
  ];
}
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${t("common.settings", "Settings")}</div>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${hasChanges ? html`
              <span class="config-changes-badge">${props.formMode === "raw" ? t("config.unsavedChanges", "Unsaved changes") : `${diff.length} ${t("config.unsavedChangeCount", "unsaved change(s)")}`}</span>
            ` : html`
          ...
          <div class="config-actions__right">
            <button class="btn btn--sm" ?disabled=${props.loading} @click=${props.onReload}>
              ${props.loading ? t("common.loading", "Loading…") : t("config.reload", "Reload")}
            </button>
            <button
        ...
        ${hasChanges && props.formMode === "form" ? html`
          <details class="config-diff">
            <summary class="config-diff__summary">
              <span>${t("config.viewPendingChanges", "View pending changes")} (${diff.length})</span>
            ...
            ? html`
                ...
                ${formUnsafe
                  ? html`<div class="callout danger" style="margin-top: 12px;">
                      ${t("config.formUnsafe", "Form view can't safely edit some fields. Use Raw to avoid losing config entries.")}
                    </div>`
                  : nothing}
              `
            : html`
